...
class DrawPlagram(Gtk.DrawingArea):
    ...
    def build_menu(self):
        ...
        menu_item.show()
        menu_item = Gtk.MenuItem(label=_('Commutar años/edad'))
        self.menu.append(menu_item)
        menu_item.connect("activate", self.on_menuitem_activate)
        menu_item.show()
    ...
    def on_menuitem_activate(self,menuitem):
        if menuitem.get_child().get_text() == _('Exportar a imagen'):
            self.png_export()
        elif menuitem.get_child().get_text() == _('Commutar años/edad'):
            self.drawer.useagecircle = not self.drawer.useagecircle 
        elif menuitem.get_child().get_text() == _('Exportar a PDF'):
            self.pdf_export()
        self.redraw()

    def on_check_toggled(self,menuitem):
        if menuitem.get_child().get_text() == _('Ver puntos de sombra'):
            if menuitem.get_active():
                self.drawer.shadow = True
            else:
                self.drawer.shadow = False
        elif menuitem.get_child().get_text() == _('Ver lineas personales'):
            if menuitem.get_active():
                self.drawer.personlines = True
            else:
                self.drawer.personlines = False
        elif menuitem.get_child().get_text() == _('Ver puntos de cambio'):
            if menuitem.get_active():
                self.drawer.turnpoints = True
            else:
                self.drawer.turnpoints = False
        elif menuitem.get_child().get_text() == _('Ver puntos de cruce'):
            if menuitem.get_active():
    ...
    def on_da_clicked(self,da,event):
        ...
        if event.type == Gdk.EventType.DOUBLE_BUTTON_PRESS and event.button == 1:
            ...
            if info['button'] < 0:
                return False
            #x, y, state = da.get_window().get_pointer() # ensure more events
            x, y = event.x, event.y
            x -= info['click_x']
            y -= info['click_y']
